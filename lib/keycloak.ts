...
export class ContainerService extends Construct {
  ...
  constructor(scope: Construct, id: string, props: ContainerServiceProps) {
    ...
    const zone = route53.HostedZone.fromLookup(cdk.Stack.of(this), 'myZone', {
      domainName: <string>process.env.R53_ZONE_NAME
    });

    const aName = new route53.ARecord(cdk.Stack.of(this), 'AliasRecord', {
      zone,
      target: route53.RecordTarget.fromAlias(new route53targets.LoadBalancerTarget(alb)),
      recordName: <string>process.env.R53_DOMAIN_NAME
